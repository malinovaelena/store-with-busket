  ...
  <div class="busket">
    <div class="busket-table">
      ...
      <div class="busket-table__body">
        <div
          class="busket-table__row"
          v-for="item in dataBusket"
          :key="item.id"
        >
          ...
          <div class="col amout">
            <label>
              <input
                v-model="item.amount"
                @input="formatted($event, item.id)"
                @change="updateAmount($event, item.id)"
              />
              шт.
            </label>
          </div>
          <div class="col price">
            {{ (item.price * item.currency).toFixed(2) }} руб/ шт.
          </div>
          <div class="col">
    ...
    <div class="total">
      <div v-if="totalAmount > 0" class="total-amount" :class="currentClass">
...
export default {
  ...
  setup() {
    ...
    watch(
      ...
      (current, old) => {
        ...
        dataBusket.value.forEach((item) => {
          totalAmount.value +=
            (item.price * item.currency).toFixed(2) * item.amount;
        });
      },
      { deep: true }
    );
    ...
    const formatted = (event, id) => {
      ...
      if (parseInt(event.target.value) > item.leftover)
        item.amount = item.leftover;
    };
